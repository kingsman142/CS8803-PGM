...
class Factor():
    ...
    def mult_factor_symptom(self, newFactor):
        ...
        if len(self.table.shape) == len(self.diseasesLeft) and len(newTable.shape) == len(self.diseasesLeft):
            ...
            newVars = self.diseasesLeft
            newVars += self.inputVars if not len(self.table.shape) == len(self.diseasesLeft) else []
            newVars += newFactor.inputVars if not len(newTable.shape) == len(self.diseasesLeft) else []
        #print(newTable.shape)
        #print(self.table.shape)
        #print(newMessageTable.shape)

        for i in range(0, int(2**len(self.diseasesLeft))):
            assignments = toBinaryArray(i, len(self.diseasesLeft)) # currently storing assignments for x1, x2, x3, x4, and x10
...
for i in range(5, 12): # 5 to 11 inclusive
    ...
    newFactor = Factor(symptoms[i-5], condVars = diseasesIndices, inputVars = [i], givenVars = givens, symptomsInit = True)
